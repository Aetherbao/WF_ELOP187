...
const dt_Temp_u8p0bn40 Gu8_facPLLCtlKp_MAP_tPCBTempSubs_X[4] = {0U, 40U, 80U,
                                                                130U};
const dt_Standardize_s16p15b0 Gs16_facPLLCtlKp_MAP_nActElecSpd_Y[4] = {
    1000U, 3276U, 10000U, 20000U};
// const uint16 Gu16_facPLLCtlKp_MAP[4][4] =
// {{320,320,320,320},{320,320,320,320},{320,320,320,320},{320,320,320,320}} ;
const uint16 Gu16_facPLLCtlKp_MAP[4] = 
                                    {1200, 1200, 1200, 1200};
// const uint16 Gu16_facPLLCtlKi_MAP[4][4] =
// {{3200,3200,3200,3200},{3200,3200,3200,3200},{3200,3200,3200,3200},{3200,3200,3200,3200}}
// ;
const uint16 Gu16_facPLLCtlKi_MAP[4] = {3000, 3000, 3000, 3000};
/*OBD*/
...
const dt_Standardize_s16p15b0 Gs16_iNoLdPhCurr_CUR_nActElecSpd_X[5] = {
    14656, 15912, 16744, 18840, 20933};
...
const dt_Standardize_s16p15b0 Gs16_iColdStrtPhCurrHiLvl1ConfTrs_C = 10240;
...
const dt_Standardize_s16p15b0 Gs16_nMaxMotoSpdDvtConfTrs_C = 1257;
...
const uint16 Gu16_ctMinMotoSpdDvtRcvyCnt_C = 400U;
...
const uint16 Gu16_stCtlDFPMSt_C[30] = {174U, 15U, 0U,   15U, 15U, 0U,  15U, 0U,
                                       0U,   13U, 110U, 12U, 11U, 0U,  0U,  11U,
                                       0U,   0U,  15U,  15U, 14U, 15U, 0U,  11U,
                                       0U,   0U,  14U,  14U, 0U,  0U}; // OK
...
dt_Standardize_s16p15b0 Gs16_nMinStrtUpElecSpd_C =
    1256; /* 157.07963267948966 */
...
dt_SpdCtrlIntgGain_u16p9b0 Gu16_facSpdCtlKi_C =
    7000; // 1000;//4096U;/* 0.00390625*/  //OK
...
const dt_RefSpdSlewRate_s16p12b0 Gs16_facRefSpdFallSlwRate_C =
    -4000; //  OK -8192;/* -2.44 */
const dt_RefSpdSlewRate_s16p12b0 Gs16_facRefSpdOLFallSlwRate_C =
    -410;                                                             /* 2.44 */
const dt_RefSpdSlewRate_s16p12b0 Gs16_facRefSpdOLRiseSlwRate_C = 410; /* 2.44 */
const dt_RefSpdSlewRate_s16p12b0 Gs16_facRefSpdRisSlwRate_C =
    8000; // 12000;//   OK 8192;/* 2.44 */
const dt_Standardize_s16p15b0 Gs16_iPhCurrLim_CUR[6] = {9344, 9344, 8064,
                                                        8064, 7296, 1408};

const dt_Temp_u8p0bn40 Gs16_iPhCurrLim_CUR_tPCBTemp_X[6] = {0U,   49U,  50U,
                                                            150U, 172U, 196U};

const dt_Standardize_s16p15b0 Gs16_iSplyCurrTempLim_CUR[6] = {3277, 3277, 3277,
                                                              3277, 3277, 3277};

const dt_Temp_u8p0bn40 Gs16_iSplyCurrTempLim_CUR_tPCBTemp_X[6] = {
    0U, 40U, 80U, 120U, 145U, 160U};

const dt_Standardize_s16p15b0 Gs16_iSplyCurrVltgLim_CUR[7] = {
    2304, 2304, 2176, 2048, 1792, 1664, 1408};

const dt_Standardize_u16p15b0 Gs16_iSplyCurrVltgLim_CUR_uSplyVltg_X[7] = {
    4608U, 5120U, 6144U, 6656U, 7168U, 8192U, 9472U};

const dt_Standardize_s16p15b0 Gs16_nAlgnRefElecSpd_C =
    50; /* 6.2831853071795862 */
...
const dt_Standardize_s16p15b0 Gu16_etaCtlBrd_MAP_ampActPhCurrAmp_X[6] = {
    1280, 2560, 3840, 5120, 6400, 7680};
const dt_Standardize_s16p15b0 Gu16_etaCtlBrd_MAP_nActElecSpd_Y[6] = {
    2094, 4188, 8377, 12565, 16754, 18848};
const dt_Standardize_u16p15b0 Gu16_etaCtlBrd_MAP[6] = 
    {48000, 49000, 47000, 44000, 44000, 44000};
...
const uint16 Gu16_tPCBTemp_CUR_ADPCBTempADC_X[38] = {
    1276, 1379, 1490, 1608, 1734, 1867, 2007, 2154, 2307, 2465,
    2627, 2792, 2959, 3125, 3289, 3449, 3605, 3755, 3896, 4029,
    4153, 4266, 4369, 4462, 4544, 4617, 4680, 4735, 4782, 4822,
    4856, 4884, 4907, 4926, 4942, 4954, 4964, 4972};
...
const dt_Temp_u8p0bn40 Gu8_tPCBTemp_CUR[38] = {
    185, 180, 175, 170, 165, 160, 155, 150, 145, 140, 135, 130, 125,
